...
export const Features= () => {
    ...
    const handleSeletedTab = (index:number) => {
        setSeletedTab(index)
    ...
    return (
        <section className="py-20 md:py-24">
            <div className="container">
                <h2 className="text-5xl md:text-6xl font-medium text-center tracking-tighter">Elevate Your OpenMic Sets!</h2>
                <p className="text-white/70 text-lg md:text-xl max-w-2xl mx-auto tracking-tight text-center mt-5">From Wrting one liner, create a build up , Framing a story narration with Audience in Sync.
                    Delivering tight bit of sets!
                </p>
                <div className="mt-10 flex flex-col lg:flex-row gap-3">
                    {tabs.map((tab, tabIndex) => (
                            <FeatureTab {...tab} selected={selectedTab == tabIndex} onClick={()=> handleSeletedTab(tabIndex)} key={tab.title} />
                    ))}
                </div>
